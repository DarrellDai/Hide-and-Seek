...
public class ItemAreaSpawner : MonoBehaviour
{
    ...
    public void StartSpawn()
    {
        ...
        {
            TerrainAndRockSetting.DestoryChildren(spawner.transform);
        }

        TerrainAndRockSetting terrainAndRockSetting = FindObjectOfType<TerrainAndRockSetting>();
        
        //Get value from TerrainAndRockSetting
        itemToSpawn = terrainAndRockSetting.itemToSpawn;
        numItemsToSpawn = terrainAndRockSetting.numberOfItemsToSpawn;
        ...
        int seed = terrainAndRockSetting.seed;
        float length = terrainAndRockSetting.CalculateMapSize();
    ...
    void PlaceToSurface(GameObject gameObject)
    {
        ...
        if (Physics.Raycast(ray, out hitInfoTerrainAndRock, 100,
                1 << LayerMask.NameToLayer("Terrain") | 1 << LayerMask.NameToLayer("Rock")))
        {
            ...
            if (Physics.Raycast(rayRock, out hitInfoRock, 100, 1 << LayerMask.NameToLayer("Rock"))) 
            {
                ...
                float offset = Vector3.Distance(gameObject.transform.position, hitInfoRock.point);
                gameObject.transform.position = hitInfoTerrainAndRock.point+offset*Vector3.up;
